...
class ProductController extends Controller
{
    ...
    public function getAdd()
    {
        ...
        return view('admin.product.add',['theloai'=>$theloai]);
    }
    ...
    public function postAdd(Request $request)
    {
        ...
        if($request->hasFile('image'))
        {
            ...
            {
                return redirect('admin/product/add')->with('thongbao','Ảnh không đúng định dạng');
            }
    ...
    public function getEdit($id)
    {
        ...
        return view("admin.product.edit",['sanpham'=>$sanpham,'theloai'=>$theloai]);
    }
    ...
    public function postEdit(Request $request, $id)
    {
        ...
        if($request->hasFile('image'))
        {
            ...
            if($duoi != 'jpg' && $duoi != 'png' && $duoi != 'jpeg')
            {
                return redirect('admin/product/edit/'.$id)->with('error_img','Bạn chọn hình chưa đúng định dạng');
            }
            $name = $file->getClientOriginalName();
            $hinh = str_random(5)."-".$name;
            while(file_exists("upload/product/".$hinh))
        ...
        $sanpham->save();
        return redirect("admin/product/edit/".$id)->with('thongbao','Bạn đã sửa sản phẩm thành công');
    }
